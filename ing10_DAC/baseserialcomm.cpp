...
/* BaudRate 波特率 在控件中列出支持的波特率 */
void BaseSerialComm::listBaudRate(QComboBox *cbbBaud, qint32 dummy)
{
    dummy = dummy;
    QMetaEnum mtaEnum = QMetaEnum::fromType<BaseSerialComm::BaudRate>();
    QString tmp;
    for (int i = 0; i < mtaEnum.keyCount(); i++) {
        /* 跳过特定值 */
        switch(mtaEnum.value(i)){
        case BaseSerialComm::BaudRate::Baud1200 :
        case BaseSerialComm::BaudRate::Baud2400 :
        case BaseSerialComm::BaudRate::Baud128000:
        case BaseSerialComm::BaudRate::Baud1000000 :
        case BaseSerialComm::BaudRate::UnknownBaud :
            continue;
            break;
        }
        tmp = QString::number(mtaEnum.value(i));
    ...
    cbbBaud->setCurrentText("57600"); // 设定默认值
}
...
/* DataBits 数据位 */
void BaseSerialComm::listDataBit(QComboBox *cbbDataBit,  qint32 dummy)
{
    dummy = dummy;
    QString tmp;
    QMetaEnum mtaEnum = QMetaEnum::fromType<BaseSerialComm::DataBits>();
    for (int i = 0; i < mtaEnum.keyCount(); i++) {
        /* 跳过特定值 */
        switch(mtaEnum.value(i)){
        case BaseSerialComm::DataBits::Data8:
            tmp = QString::number(mtaEnum.value(i));
            cbbDataBit->addItem(tmp, mtaEnum.value(i));
            break;
       default:
...
/* Parity 校验*/
void BaseSerialComm::listVerify(QComboBox *cbbVerify, qint32 dummy)
{
    dummy = dummy;
    /* 获取枚举元信息,并且与下拉列表 cbbVerify的item 绑定 */
    QMetaEnum mtaEnum = QMetaEnum::fromType<QSerialPort::Parity>();
    for (int i = 0; i < mtaEnum.keyCount(); i++) { // 遍历每个枚举值,获得text和data,添加到item中
        /* 跳过特定值 */
        switch(mtaEnum.value(i)){
        case BaseSerialComm::Parity::NoParity:
        case BaseSerialComm::Parity::EvenParity:
        case BaseSerialComm::Parity::OddParity:
            cbbVerify->addItem(mtaEnum.key(i), mtaEnum.value(i));
            break;
        default :
            continue;
    ...
    cbbVerify->setCurrentText("EvenParity"); // 设定默认值
}
